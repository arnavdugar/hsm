...
func (renderer *Renderer) createHandlerInterface() *ast.GenDecl {
	...
	for _, action := range renderer.Actions.Values {
		...
		for _, guard := range actionData.Guards {
		...
		for _, transition := range actionData.Transitions {
			methods = append(methods, &ast.Field{
				Names: []*ast.Ident{{
					Name: transition,
				}},
...
func (renderer *Renderer) createHandleTransitions(
	...
	errDefined, statusDefined := false, false
	...
	for _, transition := range transitions {
		...
		if transition.Transition != "" {
			transitionCall = append(transitionCall, &ast.AssignStmt{
				Lhs: []ast.Expr{
					&ast.Ident{
						Name: "err",
					},
				...
				Rhs: []ast.Expr{
					&ast.CallExpr{
						Fun: &ast.Ident{
							Name: fmt.Sprintf("handler.%s", transition.Transition),
						},
						Args: renderer.createHandlerArgs(action),
					},
				},
			}, &ast.IfStmt{
				Cond: &ast.BinaryExpr{
		...
		if transition.Guard != "" {
			var assignToken token.Token
			if errDefined && statusDefined {
				assignToken = token.ASSIGN
			} else {
				assignToken = token.DEFINE
				errDefined, statusDefined = true, true
			}

			transitionList = append(transitionList, &ast.AssignStmt{
				Lhs: []ast.Expr{
					&ast.Ident{
						Name: "status",
					},
...
func (renderer *Renderer) createHandlerArgs(action config.Action) []ast.Expr {
	args := []ast.Expr{}
