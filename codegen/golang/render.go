...
func (renderer *Renderer) Render() ([]byte, error) {
	...
	buffer.WriteString(`// Code generated by hsm/codegen. DO NOT EDIT.
//
...
func createImports(imports []config.Import) *ast.GenDecl {
	imports = append([]config.Import{{
		Path: "github.com/arnavdugar/hsm/runtime",
	}}, imports...)
	slices.SortFunc(imports, func(a, b config.Import) int {
		if a.Path == b.Path {
	...
	spec := make([]ast.Spec, 0, len(imports))
...
func (renderer *Renderer) createHandlerInterface() *ast.GenDecl {
	...
	for _, action := range renderer.Actions.Values {
		...
		for _, guard := range actionData.Guards.Values() {
			methods = append(methods, createGuardMethodSignature(guard, actionParamsList))
		}
			...
			&ast.ReturnStmt{
				Results: []ast.Expr{
					...
					&ast.SelectorExpr{
						...
						Sel: ast.NewIdent("ErrUnknownAction"),
					},
				},
			},
		},
				...
				List: []ast.Stmt{
					&ast.ReturnStmt{
						Results: []ast.Expr{
							...
							&ast.SelectorExpr{
								X:   ast.NewIdent("runtime"),
								Sel: ast.NewIdent("ErrInvalidActionDataType"),
							},
						},
					},
				},
			...
			Body: []ast.Stmt{
				&ast.ReturnStmt{
					Results: []ast.Expr{
						...
						&ast.SelectorExpr{
							X:   ast.NewIdent("runtime"),
							Sel: ast.NewIdent("ErrNoTransition"),
						},
					},
				},
			},
		...
		Body: []ast.Stmt{
			&ast.ReturnStmt{
				Results: []ast.Expr{
					...
					&ast.SelectorExpr{
						X:   ast.NewIdent("runtime"),
						Sel: ast.NewIdent("ErrUnknownState"),
					},
				},
			},
		},
...
func (renderer *Renderer) createHandleTransitions(
	...
	if exitState.Exit != "" {
		exitCall = renderer.createBoundaryHandler(exitState.Exit)
	}
						...
						Rhs: []ast.Expr{
							&ast.CallExpr{
								Fun: &ast.SelectorExpr{
									...
									Sel: ast.NewIdent(transition.Guard),
								},
	...
	return append(transitionList, &ast.ReturnStmt{
		Results: []ast.Expr{
			ast.NewIdent("state"),
			&ast.SelectorExpr{
				X:   ast.NewIdent("runtime"),
